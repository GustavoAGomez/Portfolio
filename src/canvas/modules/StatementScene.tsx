...
/** textDim, not numberDim — numberDim stays for the works numbers / hero stripe. */
const BEYOND_COLOR = BRAND.textDim

/** Statement WebGL layer: oversized dim ambient word behind the DOM headline. */
export function StatementScene({ id }: { id: SectionId }) {
  const { getCenter } = useSection(id)
  const { worldWidth } = useBlock()
  // 0.3125 ≡ fontSize 6 at a 1440px viewport; capped there.
